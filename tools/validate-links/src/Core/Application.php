...
final class Application
{
    ...
    private readonly LinkRectifier $linkRectifier;
    ...
    public function __construct()
    {
        ...
        $this->linkRectifier = new LinkRectifier($this->logger, $this->securityValidator);
    ...
    private function handleFixing(array $config, array $validationResults): array
    {
        ...
        $this->logger->info(sprintf('Found %d broken links, attempting to fix...', $brokenLinks));

        try {
            // Apply automatic fixes
            $validationResults = $this->linkRectifier->fixLinks($validationResults, $config);

            $fixedLinks = 0;
            foreach ($validationResults['results'] as $fileResult) {
                if (!empty($fileResult['broken_links'])) {
                    foreach ($fileResult['broken_links'] as $link) {
                        if (isset($link['fix_applied'])) {
                            $fixedLinks++;
                        }
                    }
                }
            ...
            $remainingBroken = $brokenLinks - $fixedLinks;
            $this->logger->info(sprintf('✅ Fixed %d links, %d remaining', $fixedLinks, $remainingBroken));

            if ($remainingBroken > 0) {
                $this->logger->warning('Some links could not be fixed automatically');
            }

        } catch (\Throwable $e) {
            $this->logger->error('Link fixing failed: ' . $e->getMessage());
            $this->logger->debug('Stack trace: ' . $e->getTraceAsString());
        }
