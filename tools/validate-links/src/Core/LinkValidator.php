...
final class LinkValidator
{
    ...
    public function validateFiles(
        ...
        // For cross-reference validation, we need all files in the directory structure
        $allFiles = $this->buildFileIndex($this->discoverAllFiles($files));
        $totalBrokenLinks = 0;
        $filesProcessed = 0;
    ...
    private function validateCrossReferenceLink(string $filePath, array $link, array $allFiles, bool $caseSensitive): array
    {
        ...
        // Check if target file exists using multiple strategies
        $targetExists = false;
        $actualFilePath = null;
        ...
        foreach ($allFiles as $file) {
            ...
            // Strategy 1: Direct normalized path match
            if ($this->pathsMatch($normalizedFile, $normalizedPath, $caseSensitive)) {
                $targetExists = true;
                $actualFilePath = $file;
                break;
            }

            // Strategy 2: Basename match (for same-directory references)
            if (basename($normalizedFile) === basename($normalizedPath) &&
            ...
            // Strategy 3: Case-insensitive basename match
            if (!$caseSensitive &&
                strtolower(basename($normalizedFile)) === strtolower(basename($normalizedPath))) {
                $targetExists = true;
                $actualFilePath = $file;
                break;
            }
        ...
        if (!$targetExists) {
            ...
            $this->logger->debug("Cross-reference validation failed:");
            $this->logger->debug("  Base directory: {$baseDir}");
            $this->logger->debug("  Target path: {$targetPath}");
            $this->logger->debug("  Resolved path: {$fullPath}");
            $this->logger->debug("  Normalized path: {$normalizedPath}");
            $this->logger->debug("  Available files: " . implode(', ', array_slice($allFiles, 0, 5)));
    ...
    /**
     * Discover all files in the directory structure for cross-reference validation
     *
     * @param array<string> $initialFiles Initial files provided by user
     * @return array<string> All files discovered in the directory structure
     */
    private function discoverAllFiles(array $initialFiles): array
    {
        $allFiles = [];
        ...
        foreach ($initialFiles as $file) {
            if (is_file($file)) {
                // If this is a file, add it and also scan its directory
                $allFiles[] = $file;
                $directory = dirname($file);

                if (!in_array($directory, $processedDirectories)) {
                    $directoryFiles = $this->findFiles($directory, 0, false, false, []);
                    $allFiles = array_merge($allFiles, $directoryFiles);
                    $processedDirectories[] = $directory;
                }
            } elseif (is_dir($file)) {
                // If this is a directory, scan it recursively
                if (!in_array($file, $processedDirectories)) {
                    $directoryFiles = $this->findFiles($file, 0, false, false, []);
                    $allFiles = array_merge($allFiles, $directoryFiles);
                    $processedDirectories[] = $file;
                }
            }
        }
